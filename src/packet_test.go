...
func TestServerBoundHandshak(t *testing.T) {
	tt := []struct {
	...
	}{
		{
			...
			expectedRawData: []byte{0x12, 0x00, 0xC2, 0x04, 0x0B, 0x73, 0x70, 0x6F, 0x6F, 0x6B, 0x2E, 0x73, 0x70, 0x61, 0x63, 0x65, 0x63, 0xDD, 0x01},
			packet: ultravioletv2.ServerBoundHandshakePacket{
				PacketId:        0x00,
				ProtocolVersion: 578,
		...
		},
		{
			...
			expectedRawData: []byte{0x12, 0x00, 0xC2, 0x04, 0x0B, 0x65, 0x78, 0x61, 0x6D, 0x70, 0x6C, 0x65, 0x2E, 0x63, 0x6F, 0x6D, 0x05, 0x39, 0x01},
			packet: ultravioletv2.ServerBoundHandshakePacket{
				PacketId:        0x00,
				ProtocolVersion: 578,
	...
	for _, tc := range tt {
		...
		if n != int64(len(tc.expectedRawData)) {
			t.Errorf("Expected %d bytes, got %d", len(tc.rawPacket), n)
		}

		if !bytes.Equal(buf.Bytes(), tc.expectedRawData) {
